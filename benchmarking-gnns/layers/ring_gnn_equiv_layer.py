...
class RingGNNEquivLayer(nn.Module):
    ...
    def inference(self, inputs):
        ...
        # y = torch.cat(ops_out)
        # print("ops_out.shape", y.shape)
        # np.savetxt("./data_single/inter_rst/ops_out_nostack.txt", y.view(-1, y.size(-1)).numpy(), delimiter=',', fmt="%.6f")


        ops_out = torch.stack(ops_out, dim = 2)

        print("ops_out_stack.shape", ops_out.shape)
        # np.savetxt("./data_single/inter_rst/ops_out_stack.txt", ops_out.view(-1, ops_out.size(-1)).numpy(), delimiter=',', fmt="%.6f")
        ...
        y = torch.cat(output_list)
        print("output_list.shape", y.shape)
        # np.savetxt("./data_single/inter_rst/output_list.txt", y.view(-1, y.size(-1)).numpy(), delimiter=',', fmt="%.6f")
...
def ops_2_to_2(inputs, dim, normalization='inf', normalization_val=1.0): # N x D x m x m
    ...
    if normalization is not None:
        ...
        if normalization == 'inf':
            # print("ops_2_to_2 is using norm")
            ...
            print("ops_2_to_2 not using norm")
