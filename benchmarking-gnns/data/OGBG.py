...
class OGBGDataset(torch.utils.data.Dataset):
    def __init__(self, name='ogbg-molhiv'):
        t0 = time.time()
        self.name = name
        print("[!] Dataset: ", self.name)
        # creat path+folder to store dataset if not exist
        dataset_path = "./data/ogbg/" + self.name
        creat_path(dataset_path)

        self.all = DglGraphPropPredDataset(name = self.name, root=dataset_path)

        # this function splits data into train/val/test and returns the indices
        self.all_idx = self.all.get_idx_split()
        
        self.train = self.all[self.all_idx["train"]]
        self.val = self.all[self.all_idx["valid"]]
        self.test = self.all[self.all_idx["test"]]
