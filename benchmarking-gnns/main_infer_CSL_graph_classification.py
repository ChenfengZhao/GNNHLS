...
def load_module_state(model, state_name):
    """Load parameters to the target gnn model from an external file
...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs):
    ...
    # save model parameters into external file
    print("Saving param of %s network..." % MODEL_NAME)
    torch.save(model.state_dict(), './saved_data/CSL_graph_classification/gcn/%s_param' % MODEL_NAME)
...
def main():    
    ...
    model, dataset = train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs)
    ...
    test_X, test_Y = map(list, zip(*(dataset.all)))
    test_bg = dgl.batch(test_X)

    # info of the result of nomal inference
    batch_x = test_bg.ndata['feat'].to(device)
    batch_e = test_bg.edata['feat'].to(device)
    rst_org = model(test_bg, batch_x, batch_e)
    print("rst_org", type(rst_org), rst_org.size(), rst_org.dtype)

    # load parameters to the model
    pretrained_dict = load_module_state(model, './saved_data/CSL_graph_classification/gcn/%s_param' % MODEL_NAME)
    # print(pretrained_dict)
    for key, vaule in pretrained_dict.items():
        print('saved_param: ', key, vaule.size(), type(vaule))
    
    gnn_type = "gcn"

    # save weight of conv1 to external file
    ...
    np.savetxt("./saved_data/CSL_graph_classification/%s/weight_l0.txt" % gnn_type, pretrained_dict["layers.0.conv.weight"].numpy(), delimiter=',')

    # ****** print info of graphs ******
    print("total vertex # of bg:", test_bg.number_of_nodes())
    print("total vertex # of bg:", test_bg.number_of_edges())

    # convert bg to csr format
    if dgl.__version__ < "0.5":
        g_csr = test_bg.adjacency_matrix_scipy(transpose=False, fmt="csr", return_edge_ids=False)
    else:
        g_csr = test_bg.adj(transpose=False, scipy_fmt="csr")

    # indptr
    # np.savetxt("./saved_data/CSL_graph_classification/%s/csr_indptr.txt" % gnn_type, g_csr.indptr, fmt='%u', delimiter=",")
    ...
    np.savetxt("./saved_data/CSL_graph_classification/%s/csr_indptr_trans.txt" % gnn_type, c, fmt='%u', delimiter=",")

    # indices
    np.savetxt("./saved_data/CSL_graph_classification/%s/csr_indices.txt" % gnn_type, g_csr.indices, fmt='%u', delimiter=",")
    # data
    np.savetxt("./saved_data/CSL_graph_classification/%s/csr_data.txt" % gnn_type, g_csr.data, delimiter=",")

    # in_degree
    np.savetxt("./saved_data/CSL_graph_classification/%s/indeg.txt" % gnn_type, test_bg.in_degrees().numpy(), fmt='%u', delimiter=",")

    # chech the correctness of pretrained model
    with torch.no_grad():
        rst = model.inference(test_bg, batch_x, batch_e)
        print("whether pretrained model is correct:", torch.equal(rst, rst_org))
