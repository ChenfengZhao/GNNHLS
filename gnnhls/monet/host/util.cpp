...
/**
 * @brief Check whether two array/matrix are the same. If not, show node idx (nidx) and feat idx (featidx).
 * 
 * @param n_nodes Total number of node
 * @param feats_out feature size of each node
 * @param rst Calculated result array
 * @param cor_rst Correct result array
 */
int check_rst_v2(uint64_t n_nodes, uint64_t feats_out, TYPE* rst, TYPE* cor_rst)
{
    ...
    for(uint64_t n=0; n< n_nodes; n++){
        for(uint64_t fo=0; fo<feats_out; fo++){

        #ifdef FLOAT32
            if(!fequal(rst[n*feats_out + fo], cor_rst[n*feats_out + fo]))
        #else
            if(!dequal(rst[n*feats_out + fo], cor_rst[n*feats_out + fo]))
        #endif
            {
                err_flag += 1;
            #ifdef FLOAT32
                // printf("rst[%llu] incorrect: %f, %f\n", n*feats_out + fo, rst[n*feats_out + fo], cor_rst[n*feats_out + fo]);
                if(err_flag <= FEATS_OUT*2){
                    printf("rst[nidx: %llu, featidx: %llu] incorrect: %.8f, %f\n", n, fo, rst[n*feats_out + fo], cor_rst[n*feats_out + fo]);
                }
            #else
                printf("rst[nidx: %llu, featidx: %llu] incorrect: %.15lf, %.15lf\n", n, fo, rst[n*feats_out + fo], cor_rst[n*feats_out + fo]);
            #endif
            }
        }
    }
...
/**
 * @brief Check whether two array/matrix are the same. Only checking the node features that are generated by HLS kernel. If not, show node idx (nidx) and feat idx (featidx).
 * 
 * @param indptr pointer array of the graph.
 * @param n_nodes Total number of node
 * @param feats_out feature size of each node
 * @param rst Calculated result array
 * @param cor_rst Correct result array
 */
int check_rst_v2_graph(uint64_t* indptr, uint64_t n_nodes, uint64_t feats_out, TYPE* rst, TYPE* cor_rst)
{
    using namespace std;
    ...
    uint64_t err_flag = 0;
    for(uint64_t n=0; n< n_nodes; n++){
        edge_index_t edge_begin = indptr[n * 2];
        edge_index_t edge_end = indptr[n * 2 + 1];
